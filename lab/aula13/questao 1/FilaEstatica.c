...
void imprime_Fila(Fila *fi)
{
    ...
    //Imprime o �ltimo da fila
    imprime_Aluno(fi->dados[i]);
}

int RemEsq(Fila *deque)
{
    printf("%i %i\n", deque->inicio, deque->final);
    if (deque != NULL && !Fila_vazia(deque))
    {
...
{
    if (deque != NULL && !Fila_cheia(deque))
    {
        deque->inicio--;
        if (deque->inicio < 0)
        {
        ...
        deque->dados[deque->inicio] = aluno;
        deque->qtd++;
        return 1;
    }
